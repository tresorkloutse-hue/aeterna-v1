    ...
    try {
      ...
      const checkoutRes = await fetch('/api/create-checkout', {
        ...
        body: JSON.stringify({
          ...
          title:           form.title,
          message:         form.message || 'Sanctuaire créé avec AETERNA',
          audio_url:       audioUrl,
      ...
      console.log('[Checkout]', checkoutRes.status, responseData)

      if (!checkoutRes.ok || responseData.error) {
        throw new Error(responseData.error ?? 'Erreur serveur (' + checkoutRes.status + ')')
      }

      const { sessionId } = responseData
      if (!sessionId) throw new Error('Session Stripe non créée')
          ...
          {step===3 && (
            <motion.div key="s3" variants={variants} initial="enter" animate="center" exit="exit" transition={ease}>
              ...
              <div style={{ border:'1px solid rgba(224,196,180,.14)', padding:'36px', marginBottom:28 }}>
                {[
                  ['Protocole', <span key="p" style={{ fontFamily:"'Cinzel Decorative',serif", fontSize:10, color:selectedPlan.accent }}>{selectedPlan.glyph} {selectedPlan.name}</span>],
                  ['De',        form.sender_name],
              ...
              {error && (
                <p style={{ fontSize:13, color:'rgba(239,68,68,.8)', marginBottom:16, fontFamily:"'Jost',sans-serif", padding:'12px', border:'1px solid rgba(239,68,68,.3)', background:'rgba(239,68,68,.05)' }}>
                  {error}
                </p>
              )}

              <p style={{ fontSize:10, color:'rgba(247,242,236,.18)', textAlign:'center', lineHeight:1.85, marginBottom:28, fontFamily:"'Jost',sans-serif" }}>
